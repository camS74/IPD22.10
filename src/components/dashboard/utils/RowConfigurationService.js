...
export class RowConfigurationService {
  ...
  /**
   * Gets all separator rows
   * @param {Array} divisionData - The division data array
   * @returns {Array} Array of separator row configurations
   */
  static getSeparatorRows(divisionData = []) {
    const rows = this.getRowConfiguration(divisionData);
    return rows.filter(row => row.isSeparator);
  }
  ...
  /**
   * Checks if a row should have bold styling
   * @param {string} rowLabel - The row label to check
   * @returns {boolean} True if row should be bold, false otherwise
   */
  static shouldBeBold(rowLabel) {
    const boldRows = [
      'Margin over Material',
      'Actual Direct Cost Spent',
      'Dir.Cost of goods sold',
      'Direct cost as % of C.O.G.S',
    ...
    return boldRows.some(boldRow => rowLabel.includes(boldRow));
  }
