...
export const SalesCountryProvider = ({ children }) => {
  ...
  const fetchCountries = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`✅ Context: Loaded ${countryNames.length} countries for ${selectedDivision}`);
      } else {
  ...
  const fetchSalesForPeriod = useCallback(async (column) => {
    ...
    if (pendingRequests.current.has(periodKey)) {
      console.log(`⏳ Context: Already fetching data for ${periodKey}`);
      return;
    }
    
    // Check if data already exists
    if (salesData.has(periodKey)) {
      console.log(`✅ Context: Using cached data for ${periodKey}`);
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`✅ Context: Loaded sales data for ${periodKey}: ${result.data.length} countries`);
      } else {
  ...
  useEffect(() => {
    if (columnOrder.length > 0 && countries.length > 0) {
      ...
      if (periodsToFetch.length > 0) {
        console.log(`📊 Context: Fetching ${periodsToFetch.length} new periods`);
