...
class HCMDataService {
  constructor() {
    this.pool = pool;
  }

  // Get unique product groups from hcm_data_excel, excluding specified categories
  ...
  async getExcludedProductGroups() {
    try {
      const result = await this.pool.query(
        'SELECT config_value FROM hcm_master_config WHERE config_key = $1',
      ...
      console.error('Error fetching HCM excluded product groups:', error);
      return ['Service Charges', 'Services Charges', 'Others', 'Other', 'Miscellaneous', 'Service', 'Charges'];
    }
  }

  // Get all material percentages for HCM
  async getMaterialPercentages() {
    try {
      const result = await this.pool.query(
        'SELECT * FROM hcm_material_percentages ORDER BY product_group'
      );
      return result.rows;
    } catch (error) {
  ...
  async saveMaterialPercentage(productGroup, percentages) {
    try {
      ...
      const query = `
        INSERT INTO hcm_material_percentages 
      ...
      console.error('Error saving HCM material percentage:', error);
      throw error;
    }
  }
...
module.exports = new HCMDataService();
