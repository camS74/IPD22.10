...
class SBDataService {
  ...
  // Get unique product groups from sb_data_excel, excluding specified categories
  ...
  async getExcludedProductGroups() {
    try {
      const result = await this.pool.query(
        'SELECT config_value FROM sb_master_config WHERE config_key = $1',
      ...
      console.error('Error fetching SB excluded product groups:', error);
      return ['Service Charges', 'Services Charges', 'Others', 'Other', 'Miscellaneous', 'Service', 'Charges'];
    }
  }

  // Get all material percentages for SB
  async getMaterialPercentages() {
    try {
      const result = await this.pool.query(
        'SELECT * FROM sb_material_percentages ORDER BY product_group'
      );
      return result.rows;
    } catch (error) {
  ...
  async saveMaterialPercentage(productGroup, percentages) {
    try {
      ...
      console.error('Error saving SB material percentage:', error);
      throw error;
    }
  }
...
module.exports = new SBDataService();
